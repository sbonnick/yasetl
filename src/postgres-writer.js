...
class PostgressWriter {
  ...
  async create () {
    ...
    Object.keys(this.fields).forEach(name => {
      ...
      let data = `${name} TEXT`
      if (isObject(field)) {
        if ('datatype' in field) { data = `${name} ${field.datatype.toUpperCase()}` }

        if ('primary' in field && field.primary === true) { data += ' PRIMARY KEY' }
      }
      schema.push(data)
    })
  ...
  async insert (data) {
    let inserts = data.map(async item => this.insertItem(item))
  ...
  _buildInsertQuery (data) {
    ...
    Object.keys(data).forEach(name => {
      ...
      if (isObject(this.fields[name]) && 'datatype' in this.fields[name]) {
        if (this.fields[name].datatype === 'integer' || this.fields[name].datatype === 'real') { value = data[name] }
        if (this.fields[name].datatype === 'timestamptz') { value = "'" + moment(data[name]).tz('America/Los_Angeles').format() + "'" }
        if (this.fields[name].datatype === 'boolean') { value = (String(data[name]).toLowerCase() === 'true') }
      }
